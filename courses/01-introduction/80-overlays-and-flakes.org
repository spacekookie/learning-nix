...
\\
   ...
   You _can_ override packagse without an overlay, for example in-line
...
\\
   ...
   + ~override~ overrides attributes _in_ a derivation
   + ~overrideAttrs~ overrides attributes passed _to_ ~mkDerivation~

** 

   Let's put it all together and override htop!
...
\\
   ...
   + Instead of loading ~<nixpkgs>~ directly, load your own
     ~default.nix~
   + Then load ~<nixpkgs>~ and attach the ovelay
   + This doesn't make ~nix-shell~ work though :(
